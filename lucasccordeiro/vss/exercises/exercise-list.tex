...
\makeatletter\@addtoreset{exc}{}\makeatother
...
\begin{parts}
  ...
  \item Run \file{ex1.cpp} at \flag{--unwind 5} twice: once with
        \flag{--no-unwinding-assertions} and once without. Explain the two
        different verdicts, and say which of the two runs you would be willing
        to put in a safety argument.
...
\begin{parts}
  ...
  \item \file{loop-invariant2.py} reports \texttt{VERIFICATION FAILED} even
        though the original program is correct. Name the unsoundness this
        illustrates.
...
\begin{parts}
  \item Find the defect in \file{examples/ex1.cpp} by inspection, then confirm
        it with \texttt{esbmc ex1.cpp --unwind 5 --no-unwinding-assertions}.
        Add \flag{--multi-property} and list every property \esbmc{} reports on
        line 7.
...
\begin{parts}
  \item \file{examples/interleavings.c} runs \texttt{x++} twice against
        \texttt{x--} twice with no synchronisation. Enumerate the possible final
        values of \texttt{x} and justify each with an interleaving of the
        load/add/store steps.
  ...
  \item Read the \emph{Schedule reduction} line at the end of the output. What
        do \texttt{schedules\_explored} and \texttt{pruned\_by\_mpor} count, and
        why is the second number the reason the run terminates at all?
\end{parts}
\end{exercise}
...
\begin{parts}
  \item Specify the following three properties in LTL, using $c_1$ and $c_2$
        for ``process $i$ is in its critical section'' and $t_1$, $t_2$ for
        ``process $i$ has requested entry''.
        \begin{enumerate}[label=\roman*.,leftmargin=2.2em,topsep=2pt]
  ...
  \item Which of the three are \emph{safety} properties and which are
        \emph{liveness} properties? Which of them can a bounded run refute, and
        which can it only fail to refute?
...
\begin{parts}
  ...
  \item Remove the \texttt{pthread\_mutex\_lock}/\texttt{unlock} pair and
        re-verify. What is the smallest final value of \texttt{n} that \esbmc{}
        can justify with an interleaving?
