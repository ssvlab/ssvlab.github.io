...
pthread_mutex_t mutex_heater_status;  // Mutex for heater status
pthread_mutex_t mutex_temperature;    // Mutex for temperature sensor
pthread_mutex_t mutex_pressure;       // Mutex for temperature sensor
pthread_mutex_t mutex_valve_position; // Mutex for valve position
...
void set_valve_position(bool position) {
    pthread_mutex_lock(&mutex_valve_position);
    valve_position = position;
    printf("[DAC] Valve set to %s\n", position ? "OPEN" : "CLOSED");
    pthread_mutex_unlock(&mutex_valve_position);
}

// Simulated Heater Control (GPIO)
void set_heater(bool state) {
    pthread_mutex_lock(&mutex_heater_status);
    heater_status = state;
    printf("[DAC] Heater %s\n", state ? "ON" : "OFF");
    pthread_mutex_unlock(&mutex_heater_status);
}
...
void *pressure_process(void *arg) {
    while (1) {
        pthread_mutex_lock(&mutex_pressure);
        pressure = read_pressure_adc();
        printf("[P Process] Pressure: %.2f bar\n", pressure);
        pthread_mutex_unlock(&mutex_pressure);
...
int main() {
    ...
    pthread_mutex_init(&mutex_heater_status, NULL);
    pthread_mutex_init(&mutex_temperature, NULL);
    pthread_mutex_init(&mutex_pressure, NULL);
    pthread_mutex_init(&mutex_valve_position, NULL);
    ...
    pthread_mutex_destroy(&mutex_heater_status);
    pthread_mutex_destroy(&mutex_temperature);
    pthread_mutex_destroy(&mutex_pressure);
    pthread_mutex_destroy(&mutex_valve_position);
